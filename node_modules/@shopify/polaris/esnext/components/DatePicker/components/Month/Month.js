...
export function Month({ focusedDate, selected, hoverDate, disableDatesBefore, disableDatesAfter, allowRange, onChange = noop, onHover = noop, onFocus = noop, month, year, weekStartsOn, }) {
    const i18n = useI18n();
    ...
    const weekdays = getWeekdaysOrdered(weekStartsOn).map((weekday) => (<Weekday key={weekday} title={i18n.translate(`Polaris.DatePicker.daysAbbreviated.${weekdayName(weekday)}`)} current={current && new Date().getDay() === weekday} label={weekday}/>));
